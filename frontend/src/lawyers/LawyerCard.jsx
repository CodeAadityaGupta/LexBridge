...
export default function LawyerCard({ lawyer, isActive = false }) {
  ...
  return (
    <Link to={`/lawyer/${id}`} className="block select-none group h-full">
      <div
        className={`flex flex-col justify-between p-5 rounded-xl shadow-card hover:shadow-modal transition-all duration-300 cursor-pointer h-full relative overflow-hidden bg-card hover:-translate-y-[2px] active:scale-[0.99]
          ${isActive 
            ? 'ring-2 ring-accent' 
            : ''
          }
      ...
      >
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="relative">
              <Avatar name={name} src={avatar} size="md" className="shadow-sm shrink-0" />
            ...
            {/* Rating badge */}
            <div className="flex items-center space-x-1 text-[11px] text-ink font-bold bg-surface/90 px-2.5 py-0.5 rounded-full shadow-sm shrink-0">
          ...
          {specialties.length > 1 && (
            <div className="flex flex-wrap gap-1 pt-1">
              {specialties.slice(1, 3).map((spec, i) => (
                <span 
                  key={i} 
                  className="px-2 py-0.5 rounded bg-surface/70 text-[9px] font-semibold text-muted/95 tracking-wide"
                >
                  {spec}
                </span>
              ))}
        ...
        {/* Footer */}
        <div className="pt-3.5 mt-4 flex items-center justify-between text-[11px] text-muted font-sans font-semibold">
